...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		for(int i = 0; i < 65; i++) {
			sb.append(types[ThreadLocalRandom.current().nextInt(0, 5)]);
		}
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		// YOUR CODE GOES BELOW HERE
		int minIndex = population.indexOf(Collections.min(population));
		selected.add(population.get(minIndex));
		// YOUR CODE GOES ABOVE HERE
		return selected;
	}
	...
	private MyDNA pickIndividualForCrossover (ArrayList<MyDNA> population, MyDNA excludeMe)
	{
		MyDNA picked = null;
		// YOUR CODE GOES BELOW HERE
		picked = getBestIndividual(population);
		if (picked == excludeMe) {
			picked = getSecondBest(population);
		}
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		for(MyDNA dna : oldPopulation) {
			finalPopulation.add(dna);
		}
		for(MyDNA dna : newPopulation) {
			finalPopulation.add(dna);
		}

		while (finalPopulation.size() > this.getPopulationSize()) {
			int minIndex = finalPopulation.indexOf(Collections.min(finalPopulation));
			finalPopulation.remove(minIndex);
		}
	...
	private MyDNA getSecondBest (ArrayList<MyDNA> population)
	{
		MyDNA best = population.get(0);
		MyDNA secondBest = population.get(0);
		double bestFitness = Double.NEGATIVE_INFINITY;
