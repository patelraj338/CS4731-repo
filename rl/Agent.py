...
class Agent:
	...
	def updateVtable(self, newState, lastState, action, reward, terminal, availableActions):
		...
		prevTableEntry = self.v_table[self.calculateFlatState(lastState)]
		nextTableEntry = self.v_table[self.calculateFlatState(newState)]

		if terminal == True:
			prevQ = prevTableEntry[action]
			prevTableEntry[action] = prevQ + 0.1 * (reward - prevQ)
		else:
			prevQ = prevTableEntry[action]
			maxRew = nextTableEntry[0]
			maxInd = 0
			for i in range(0, len(nextTableEntry)):
				if nextTableEntry[i] > maxRew:
					maxRew = nextTableEntry[i]
					maxInd = i
			prevTableEntry[action] = prevQ + 0.1 * (reward + (0.5 * maxRew) - prevQ)
	...
	def egreedy(self, observation):
		...
		tableEntry = self.v_table[self.calculateFlatState(observation.worldState)]
		# print randNum
		# print tableEntry
		if randNum < self.epsilon:
			return random.randint(0, len(tableEntry)-1)
		else:
	...
	def greedy(self, observation):
		...
		maxRew = tableEntry[0]
		maxInd = 0
		for i in range(0, len(tableEntry)):
			if tableEntry[i] > maxRew:
				maxRew = tableEntry[i]
				maxInd = i
