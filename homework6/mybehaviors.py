...
def treeSpec(agent):
	...
	### YOUR CODE GOES BELOW HERE ###
        # spec = [(Selector, 1), [(HitpointDaemon, 0.3, 2), [(Selector, 3), [(BuffDaemon, 3, 3), [(Sequence, 4), (ChaseHero, 5), (KillHero, 6)]], [(Sequence, 7), (ChaseMinion, 8), (KillMinion, 9)]]], (Retreat, 0.5, 10)]
        # spec = [(Selector, 4), [(HitpointDaemon, 0.3, 5), \
        #     [(Selector, 6), [(DodgeDaemon, 7), (DodgeBullet, 8)], \
        #     [(Sequence, 9), (ChaseHero, 10), (ShootHero, 11)]]], \
        #      (Retreat, 0.3, 12)]
        spec = [(Selector, 4), [(HitpointDaemon, 0.3, 5), \
            [(Selector, 6), [(DodgeDaemon, 7), (DodgeBullet, 8)], \
            [(Selector, 9), [(AOEDaemon, 10), (AOEAttackHero, 11)], \
            [(Sequence, 12), (ChaseHero, 13), (ShootHero, 14)]]]], \
             (Retreat, 0.3, 15)]
...
class AOEDaemon(BTNode):
	...
	def execute(self, delta = 0):
                ...
                if self.agent.canareaeffect == True and self.target != None and \
                    distance(self.agent.getLocation(), self.target.getLocation()) < (self.agent.getRadius() * AREAEFFECTRANGE):
                    return self.getChild(0).execute(delta)
                else:
                    print "exec", self.id, "fail"
                    return False
...
class AOEChaseHero(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target == None or self.target.isAlive() == False:
			...
			print "exec", self.id, "false"
			return False
		elif distance(self.agent.getLocation(), self.target.getLocation()) < (self.agent.getRadius() * AREAEFFECTRANGE) :
			# succeeded
			print "exec", self.id, "true"
			return True
		else:
...
class AOEAttackHero(BTNode):
	...
	def execute(self, delta = 0):
		ret = BTNode.execute(self, delta)
		if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > (self.agent.getRadius() * AREAEFFECTRANGE):
			# failed executability conditions
			if self.target == None:
				print "foo none"
			else:
...
class AOEChaseMinion(BTNode):
	...
	def execute(self, delta = 0):
		...
		if self.target == None or self.target.isAlive() == False:
			...
			print "exec", self.id, "false"
			return False
		elif self.target is not None and distance(self.agent.getLocation(), self.target.getLocation()) < (self.agent.getRadius() * AREAEFFECTRANGE):
			# succeeded
			print "exec", self.id, "true"
			return True
		else:
...
class DodgeBullet(BTNode):
	def ParseArgs(self, args):
		BTNode.parseArgs(self, args)
		# First argument is the node ID
                self.target = None
                self.dodgeAngle = None
		if len(args) > 0:
			self.id = args[0]
	...
	def enter(self):
			...
			if isinstance(e, Hero):
                                ...
                                global jumpDir
                                if jumpDir == False:
                                        self.dodgeAngle =  e.orientation + 90
                                        jumpDir = True
                                elif jumpDir == True:
                                        self.dodgeAngle =  e.orientation - 90
                                        jumpDir = False
        ...
        def execute(self, delta = 0):
                ...
                if self.agent.candodge == False:
                        print "exec", self.id, "false"
                        return False
                elif self.agent.candodge == True:
                        print "exec", self.id, "true"
                        self.agent.dodge(self.dodgeAngle)
                        return True
                else:
                        self.agent.dodge(self.dodgeAngle)
                        return None
